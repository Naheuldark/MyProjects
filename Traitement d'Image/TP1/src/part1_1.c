...
{  
    ...
    int nl, nc, nl_origin, nc_origin;
    unsigned char **in = NULL, **out = NULL, **in_origin = NULL;
    ...
    printf("PSNR entre formes2.pgm et :\n");

    in_origin = lectureimagepgm("../in/formes2.pgm", &nl_origin, &nc_origin);
   
    out = filter(in, nl, nc, 0.5);
    ecritureimagepgm(strcat(av[2],"s=0.5"),
                     crop(out, 0, 0, nl, nc),
                     nl, nc);
    printf("# formes2bb67(s=0.5).pgm \t--> %f\n", PSNR(in_origin, out, nl, nc));

    out = filter(in, nl, nc, 1);
    ecritureimagepgm(strcat(av[2],"s=1"),
                     crop(out, 0, 0, nl, nc),
                     nl, nc);
    printf("# formes2bb67(s=1).pgm \t--> %f\n", PSNR(in_origin, out, nl, nc));

    out = filter(in, nl, nc, 5);
    ecritureimagepgm(strcat(av[2],"s=5"),
                     crop(out, 0, 0, nl, nc),
                     nl, nc);
    printf("# formes2bb67(s=5).pgm \t--> %f\n", PSNR(in_origin, out, nl, nc));

    out = filter(in, nl, nc, 10);
    ecritureimagepgm(strcat(av[2],"s=10"),
                     crop(out, 0, 0, nl, nc),
                     nl, nc);
    printf("# formes2bb67(s=10).pgm \t--> %f\n", PSNR(in_origin, out, nl, nc));
