...
void initialize_practical_03_scene(Viewer& viewer)
{
    ...
    //Torso
    std::shared_ptr<CylinderRenderable> torso
        = std::make_shared<CylinderRenderable>(flatShader);
    scaleM = glm::scale(glm::mat4(1.0), glm::vec3(0.5,0.5,2.0));
    torso->setLocalTransform(scaleM*torso->getModelMatrix());

    //Head
    std::shared_ptr<MeshRenderable> head
    ...
    //Left arm
    std::shared_ptr<CylinderRenderable> leftArm
        = std::make_shared<CylinderRenderable>(flatShader);
    ...
    //Left hand
    std::shared_ptr<MeshRenderable> leftHand
        = std::make_shared<MeshRenderable>(flatShader, "./../meshes/suzanne.obj");
    ...
    //Right arm
    std::shared_ptr<CylinderRenderable> rightArm
        = std::make_shared<CylinderRenderable>(flatShader);
    rotationM = glm::rotate(glm::mat4(1.0), (float)(M_PI/2.0), glm::vec3(0,1,0));
    translationM = glm::translate(glm::mat4(1.0), glm::vec3(0.0,0.0,2.0));
    ...
    //Right hand
    std::shared_ptr<MeshRenderable> rightHand
        = std::make_shared<MeshRenderable>(flatShader, "./../meshes/suzanne.obj");
    translationM = glm::translate(glm::mat4(1.0), glm::vec3(2.5,0,0));
