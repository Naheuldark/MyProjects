...
void ControlledForceFieldRenderable::do_draw()
{
    ...
    for (ParticlePtr p : particles) {
        float preAngle = p->getAngleTheta();
        p->setAngleTheta(m_status.angle);
        float nextAngle = p->getAngleTheta();
        if (preAngle!= nextAngle){
            if ((p->getAngleTheta() == M_PI/2) || (p->getAngleTheta() == -M_PI/2)){
                p->setVelocity(glm::vec3(0,p->getVelocity().y*0.5,p->getVelocity().z));
            } else {
                const glm::vec3 dimVitesse = glm::vec3(p->getVelocity().x *0.95, p->getVelocity().y*0.95, p->getVelocity().z*0.95);
                p->setVelocity(dimVitesse);
            }
        }
        // m_positions.push_back(p->getPosition());
