...
const std::vector<ParticleSkierPtr>& DynamicSystem::getParticlesSkier() const
{
    return m_particlesSkier;
}
...
void DynamicSystem::setParticlesSkier(const std::vector<ParticleSkierPtr> &particlesSkier)
{
    m_particlesSkier = particlesSkier;
}
...
void DynamicSystem::addParticleSkier(ParticleSkierPtr pS)
{
    m_particlesSkier.push_back(pS);
}
...
void DynamicSystem::detectCollisions()
{
    ...
    //Detect particle side collisions
    for (size_t i = 0; i < m_particlesSkier.size(); ++i) {
        ParticleSkierPtr p1 = m_particlesSkier[i];
        if (testParticleSide(p1, true)) {
            ParticleSideCollisionPtr c =
                std::make_shared<ParticleSideCollision>(p1, true, m_restitution);
            m_collisions.push_back(c);
        }
        if (testParticleSide(p1, false)) {
            ParticleSideCollisionPtr c =
                std::make_shared<ParticleSideCollision>(p1, false, m_restitution);
            m_collisions.push_back(c);
        }
    }
}
