...
static void initializeGL()
{
    ...
    glcheck(glClearColor(176.0/256, 224.0/256, 230.0/256,1.0f));
...
Viewer::Viewer(float width, float height) :
    ...
    m_chronoTime{0}, m_gameover{false},
...
void Viewer::draw()
{
    ...
    }
    {
        ...
        ss2 << "Temps: " << std::setprecision( 2 ) << std::fixed << getChrono();
        m_tengine.render( ss2.str(), glm::vec2(m_window.getSize().x - 1250, m_window.getSize().y - 30), glm::vec3(0.1,0.1,0.1) );
    }
    if( m_helpDisplayed )
        m_tengine.render( g_help_message, glm::vec2(100, 650), glm::vec3{.0, .1, .2});
    if (m_gameover) {
        std::ostringstream final;
        final << "\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~  BRAVO  ~~~~~~~~~~~~~~~~\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n\nYou reached the bottom of the slope in " << std::setprecision( 2 ) << std::fixed << getChrono() << "sec,\nbut unfortunately princess Peach is not there >:-3\n\n                   BWHAHAHAH  --  Bowser";
        m_tengine.render(final.str(), glm::vec2(440, 560), glm::vec3{.0,.1,.2});
    }
    ...
    if (!m_pause)
        pause << "";
    m_tengine.render(pause.str(), glm::vec2(440, 560), glm::vec3{.0,.1,.2});
}

float Viewer::getChrono()
{
    if( m_animationIsStarted && !m_gameover) {
        m_chronoTime += Duration(clock::now() - m_lastChronoTimePoint).count();
        m_lastChronoTimePoint = clock::now();
    }
    return m_chronoTime;
}

void Viewer::stopChrono()
{
    m_gameover = true;
}

void Viewer::resetChrono()
{
    m_lastChronoTimePoint = clock::now();
    m_chronoTime = 0;
    m_gameover = false;
...
void Viewer::rmRenderable(RenderablePtr r)
{
